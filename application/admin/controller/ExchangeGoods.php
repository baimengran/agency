...
class ExchangeGoods extends Base
{
    ...
    public function index()
    {
        try {
            ...
            $data = $data->where('delete_time', 0)->where('type', 3)
                ->order('status asc,create_time', 'desc')->paginate(20);
            if ($data) {
                return view('index', [
                    'val' => $key,
                    'data' => $data,
                    'empty' => '<tr><td colspan="6" align="center"><span>暂无数据</span></td></tr>'
                ]);
            }
        } catch (\Exception $e) {
    ...
    public function status(Request $request)
    {
        ...
        try {
            ...
            if ($form['status'] == 2) {
                ...
                $json = ['status' => 1, 'code' => 1, 'msg' => '以调货', 'sleep' => 5000];
            }
            Db::commit();
            return json($json);
        } catch (\Exception $e) {
...
//        dump($product_inc);die;
        Db::startTrans();
        try {
            $order = OrderGoodsModel::update(['id' => $form['id'], 'status' => 1]);
