...
class OrderGoods extends Base
{
    ...
    public function index()
    {
        try {
            ...
            $data = $data->where('delete_time', 0)->where('type',2)
...
//                return json(['code' => 0, 'msg' => '以取消', 'sleep' => 5000]);
            ...
            if ($form['status'] == 1) {
                ...
                if($order['num']!=null){
                    $myProduct = MyProduct::where('user_id', $order['user_id'])->where('product_id', $order['product_id'])
                        ->setInc('num', $order['num']);;
                }
                if($order['box_num']!=null){
                    $product_num = Db::name('product')->where('id',$order['product_id'])->value('num');
                    $num=$order['box_num']*$product_num;
                    $myProduct = MyProduct::where('user_id', $order['user_id'])->where('product_id', $order['product_id'])
                        ->setInc('box_num', $order['box_num']);;
                    $myProduct = MyProduct::where('user_id', $order['user_id'])->where('product_id', $order['product_id'])
                        ->setInc('num', $num);;
                }

                $json = ['status' => 1, 'code' => 1, 'msg' => '以补货', 'sleep' => 5000];
            }
//            if($form['status']==2){
//                $order_update = $order->update(['id' => $form['id'], 'status' => 1]);
//                $json = ['status' => 1, 'code' => 1, 'msg' => '以补货', 'sleep' => 5000];
